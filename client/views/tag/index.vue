...
export default {
  ...
  methods: {
    async _getTagList () {
      const { data: { code, data, page } } = await tag.getTagList(this.params)
      if (code === 0) {
        this.data = data
        this.page = page
      }
    ...
    async _Add (params) {
      try {
        const { data: { code, msg } } = await tag.add(params)
        if (code === 0) {
    ...
    async _Update (params) {
      try {
        const { data: { code, msg } } = await tag.update(params)
        if (code === 0) {
          this.$Message.success(msg)
          this.isModalShow = false
          this._getTagList()
    ...
    async _Delete (params) {
      const { data: { code, msg } } = await tag._delete(params)
      try {
        if (code === 0) {
          this.$Message.success(msg)
          this._getTagList()
